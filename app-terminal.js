...
function entrySafety(x,e,opts={}){
  ...
  // Rotation validates a replacement before closing one existing position.
  // In that pre-close phase one slot is intentionally reserved for the replacement.
  if(!opts.allowRotationSlot&&engineOpenCount(e)>=engineLimit(e))return{ok:false,reason:e+' position limit reached'};
...
async function testnetPreflight(x,e){
  ...
  const gate=entrySafety(x,e,{allowRotationSlot:true});
  if(!gate.ok){
...
/* ===== Profit Rotation ===== */
function rotationStrength(x,e){
  if(!x)return 0;
  const q=N(x.qualityScore);
  const sig=e==='MOMENTUM'?x.momentum:x.scalp;
  const tf=e==='MOMENTUM'
    ?((x.momentum!=='WAIT'?25:0)+(x.momentum===x.scalp&&x.scalp!=='WAIT'?10:0))
    :((x.scalp!=='WAIT'?25:0)+(x.momentum===x.scalp&&x.momentum!=='WAIT'?10:0));
  const vol=Math.min(15,Math.max(0,(N(x.pipelineVol)-VOL_FILTER)*25));
  const move=Math.min(10,Math.abs(N(x.c))*8);
  return Math.round(q*.40+tf+vol+move+(sig==='WAIT'?0:10));
}
function rotationImprovement(newRow,oldPos,e){
  const ns=rotationStrength(newRow,e);
  const oldScore=N(oldPos?.qualityScore);
  const oldStrength=oldPos?.rotationScore!=null?N(oldPos.rotationScore):oldScore;
  return{newScore:ns,oldScore:oldStrength,delta:ns-oldStrength};
}
  ...
  for(const eng of engines){
    ...
    html+=ep.map(p=>{
      ...
      return '<div class="pos-card '+(p.side==='SELL'?'sell-side':'')+'"><div class="pos-head"><span class="pos-coin">'+E(p.s)+'</span><span class="pos-tag">'+E(p.e)+'</span><span class="pos-tag">'+E(p.side)+'</span><span class="pos-tag">'+E(p.mode||'PAPER')+'</span>'+(p.signalStage?'<span class="pos-tag">'+E(p.signalStage)+'</span>':'')+(p.sl?'<button class="btn sm red" onclick="DD.close(\''+E(p.s)+'\')">Close</button>':'')+'</div>'+
        '<div class="pos-grid">'+posField('Entry',fmtPrice(p.entry))+posField('Mark',fmtPrice(p.current))+posField('Qty',fmtQty(p.q))+posField('Notional',R(N(p.notional)||N(p.entry)*N(p.q)))+posField('Risk',p.riskPct?P(p.riskPct*100):'—')+posField('SL',p.stopPct?P(p.stopPct*100):fmtPrice(p.sl))+posField('TP',p.tpPct?P(p.tpPct*100):fmtPrice(p.tp))+posField('Protection',p.mode==='TESTNET'?(p.protectionStatus||'UNKNOWN'):'—')+posField('Age',age)+posField('Quality',N(p.qualityScore)||'—')+'</div>'+
