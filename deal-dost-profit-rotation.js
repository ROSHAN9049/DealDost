...
function openFutures(c){if(typeof window.DD?.openEngine!=='function')return false;const d=side(c),m=N(c.momentumScore),s=N(c.scalpingScore);if(m>=80&&window.DD.openEngine(c.symbol,'MOMENTUM'))return'MOMENTUM';if(s>=80&&window.DD.openEngine(c.symbol,'SCALPING'))return'SCALPING';return false}
