...
(()=>{'use strict';
 ...
 const now=()=>Date.now();
 const key='dd_profit_rotation_v1';
 const recent={};
 ...
 const strong=()=>candidates().filter(x=>x&&x.confirmed&&score(x)>=CFG.minScore&&(side(x)==='BUY'||side(x)==='SELL'))
 ...
 function profitableFutures(){
   ...
   return ps.filter(p=>p&&p.s&&N(p.entry)>0&&N(p.current)>0).map(p=>{
     const risk=Math.abs(N(p.entry)-N(p.sl))*Math.max(N(p.q),0),
       gross=String(p.side).toUpperCase()==='SELL'?(N(p.entry)-N(p.current))*N(p.q):(N(p.current)-N(p.entry))*N(p.q),
       pnl=N(p.pnl)>0?N(p.pnl):gross-N(p.entryFee);
 ...
 function closeOptionProfits(best){
   try{
     ...
     for(const p of ps){
       ...
       if(!c||score(c)<CFG.minScore)continue;
 ...
 async function rotate(){
   ...
   for(const p of ps){
     ...
     if(typeof window.DD?.close!=='function')continue;
     try{
