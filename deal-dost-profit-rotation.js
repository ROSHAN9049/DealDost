/* DealDost Unified Rotation Engine V7
 * PAPER-first. Profit + worst-loss rotation across Futures and Options.
 * A confirmed Top-Mover >= 80 is required. Only one position is rotated per cycle.
 * LIVE execution is untouched.
 */
(()=>{'use strict';
if(window.__DD_PROFIT_ROTATION_V7)return;window.__DD_PROFIT_ROTATION_V7=true;
...
function futuresPositions(){try{const q=JSON.parse(localStorage.getItem('ddv5_PAPER')||'{}'),ps=Array.isArray(q.pos)?q.pos:[];return ps.filter(p=>p&&p.s&&N(p.entry)>0&&N(p.current)>0).map(p=>{const gross=String(p.side).toUpperCase()==='SELL'?(N(p.entry)-N(p.current))*N(p.q):(N(p.current)-N(p.entry))*N(p.q);const pnl=N(p.pnl)>0?N(p.pnl):gross-N(p.entryFee);return{...p,_pnl:pnl,_kind:'FUTURES',_symbol:p.s}})}catch{return[]}}
function optionPositions(){try{const ps=window.DDOptions?.getState?.()?.positions;return(Array.isArray(ps)?ps:[]).filter(p=>p&&p.id).map(p=>({...p,_pnl:N(p.pnl),_kind:'OPTIONS',_symbol:p.u}))}catch{return[]}}
...
function panel(){const el=mount(),st=strong()[0],p=profitable(),w=worstLoss(),key=(st?[st.symbol,side(st),score(st)].join('|'):'WAIT')+'|P'+p.length+'|W'+(w?String(w._symbol):'NONE');if(key===lastKey&&el.dataset.ready==='1')return;lastKey=key;el.innerHTML='<div class="panel" style="margin:0"><div class="panel-header"><div class="panel-title">UNIFIED ROTATION V7</div><div class="panel-sub">Profit + worst-loss rotation · confirmed Top-Mover ≥ 80 · PAPER only</div></div><div style="padding:9px;font-size:11px;color:#8ea3b8">'+(st?'Next strong signal: <b>'+String(st.symbol)+'</b> · '+side(st)+' · score '+score(st):'Waiting for confirmed Top-Mover signal ≥ 80…')+' · Profitable: <b>'+p.length+'</b> · Worst loss: <b>'+(w?(String(w._symbol)+' ₹'+w._pnl.toFixed(2)):'None')+'</b></div><div style="padding:0 9px 9px;font-size:10px;color:#71859b">Priority: profitable position first; otherwise one worst-loss position. Replacement may use Momentum, Scalping or Options.</div></div>';el.dataset.ready='1'}
