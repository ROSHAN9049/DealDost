...
function controls(){
 const root=document.getElementById('dd-mode-root');if(!root)return;root.innerHTML='';
 const current=localStorage.getItem(KEY)||'PAPER';
 ['PAPER','TESTNET','LIVE'].forEach(m=>{const b=document.createElement('button');b.type='button';b.textContent=m+(m===current?' ✓':'');b.dataset.mode=m;b.style.cssText='background:'+(m===current?'#15304b':'#0d1a2a')+';color:#e9f1fb;border:1px solid '+(m===current?'#4da3ff':'#2a415b')+';border-radius:10px;padding:9px 13px;cursor:pointer;font-weight:800';b.onclick=()=>{if(m===current)return;if(m==='LIVE'&&!confirm('LIVE mode selected. Real Binance orders remain server-locked until LIVE_UNLOCKED=true. Continue?'))return;localStorage.setItem(KEY,m);location.reload()};root.appendChild(b)});
 const note=document.createElement('span');note.id='dd-mode-note';note.style.cssText='font-size:11px;color:#8295ad;margin-left:4px';note.textContent=current==='PAPER'?'Local paper simulation':current==='TESTNET'?'TESTNET · local simulation fallback (Binance Demo private API is region-restricted)':'Binance LIVE · real orders locked by server';root.appendChild(note);
}
function forceTestnetUI(){if((localStorage.getItem(KEY)||'PAPER')!=='TESTNET')return;document.querySelectorAll('body *').forEach(el=>{if(el.children.length===0&&/PAPER MODE · Real orders are OFF\./.test(el.textContent||''))el.textContent=(el.textContent||'').replace('PAPER MODE · Real orders are OFF.','TESTNET · LOCAL SIMULATION · Real orders are OFF.');});document.querySelectorAll('button').forEach(b=>{if(b.textContent.trim()==='AUTO LIVE ON')b.textContent='AUTO TESTNET ON';if(b.textContent.trim()==='AUTO LIVE OFF')b.textContent='AUTO TESTNET OFF'});}
