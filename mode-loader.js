...
function patch(src){
 ...
 src=src.replace("if(p.mode==='PAPER'){","if(['PAPER','TESTNET'].includes(p.mode)){" );
...
function forceTestnetUI(){const cur=localStorage.getItem(KEY)||'PAPER';if(cur!=='TESTNET')return;const replaceNode=(el)=>{if(!el||el.tagName==='SCRIPT'||el.tagName==='STYLE')return;const txt=(el.textContent||'').trim();if(txt.includes('PAPER MODE · Real orders are OFF.')){for(const n of el.childNodes){if(n.nodeType===3&&n.nodeValue.includes('PAPER MODE · Real orders are OFF.'))n.nodeValue=n.nodeValue.replace(/PAPER MODE · Real orders are OFF\./g,'TESTNET · LOCAL SIMULATION · Real orders are OFF.')}}};document.querySelectorAll('body *').forEach(replaceNode);document.querySelectorAll('button').forEach(b=>{const t=b.textContent.trim();if(t==='AUTO LIVE ON')b.textContent='AUTO TESTNET ON';if(t==='AUTO LIVE OFF')b.textContent='AUTO TESTNET OFF'});const n=document.getElementById('dd-mode-note');if(n)n.textContent='TESTNET · local simulation fallback (Binance Demo private API is region-restricted)';}
async function boot(){try{controls();const r=await fetch('/app-scanner-v3.js?mode='+Date.now(),{cache:'no-store'});if(!r.ok)throw Error('Scanner source failed '+r.status);let src=await r.text();src=patch(src);(0,eval)(src);controls();forceTestnetUI();setInterval(forceTestnetUI,500);const mo=new MutationObserver(forceTestnetUI);mo.observe(document.body,{subtree:true,childList:true,characterData:true});}catch(e){const a=document.getElementById('app');if(a)a.innerHTML='<div style="padding:30px"><b>Scanner startup error</b><div style="color:#ff6178;margin-top:8px">'+String(e.message||e)+'</div></div>'}}
