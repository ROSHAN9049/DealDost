...
function patch(src){
 ...
 src=src.replace("async function syncAccount(){if(Date.now()-S.lastAccount<2500)return;","async function syncAccount(){if(S.mode!=='LIVE'){S.account=null;return;}if(Date.now()-S.lastAccount<2500)return;");
 ...
 src=src.replace("const top=[...syms].sort((a,b)=>N(S.t[b]?.v)-N(S.t[a]?.v)).slice(0,60);","const top=[...syms].sort((a,b)=>Math.abs(N(S.t[b]?.c))-Math.abs(N(S.t[a]?.c))).slice(0,50);");
 return src;
}
