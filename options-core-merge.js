/* DealDost unified Futures + Options accounting bridge — stable render */
(()=>{'use strict';
...
function render(force){const app=document.querySelector('#app');if(!app||!window.DD)return;if(DD.tab!=='dashboard'&&DD.tab!=='pnl')return;let p=app.querySelector('.dd-opt-core-merge');if(!p){app.insertAdjacentHTML('beforeend',panel());return}if(force){const c=p.querySelector('.dd-opt-core-content');if(c){const fresh=document.createRange().createContextualFragment(panel());c.replaceWith(fresh.querySelector('.dd-opt-core-content'))}}}
function hook(){if(!window.DD||window.DD.__optCoreMerge)return;if(typeof DD.render==='function'){const old=DD.render;DD.render=function(){const r=old.apply(this,arguments);render(false);return r};DD.__optCoreMerge=true;render(false)}}
window.DDCombinedOptions={stats,render};hook();
})();
