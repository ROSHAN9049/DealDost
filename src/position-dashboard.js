...
function shared(symbol){try{const m=window.__PAPER_LIVE_TICKERS;if(m?.get){const x=m.get(symbol);const p=Number(x?.lastPrice);if(Number.isFinite(p)&&p>0)return p}}catch{}return null}
function connect(){
  try{if(ws&&(ws.readyState===WebSocket.OPEN||ws.readyState===WebSocket.CONNECTING))return;ws=new WebSocket('wss://fstream.binance.com/stream');
    ws.onopen=()=>ws.send(JSON.stringify({method:'SUBSCRIBE',params:['!ticker@arr'],id:88}));
    ws.onmessage=e=>{try{const m=JSON.parse(e.data),data=Array.isArray(m)?m:(Array.isArray(m.data)?m.data:[]);data.forEach(x=>{if(x?.s&&x.c)live.set(x.s,Number(x.c))})}catch{}};
    ws.onclose=()=>{clearTimeout(retry);retry=setTimeout(connect,2000)};
    ws.onerror=()=>{try{ws.close()}catch{}};
  }catch{clearTimeout(retry);retry=setTimeout(connect,2000)}
}
function render(){
