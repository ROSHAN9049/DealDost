...
function connect(){
  try{ws?.close();ws=new WebSocket('wss://fstream.binance.com/ws/!ticker@arr');
    ws.onmessage=e=>{try{const data=JSON.parse(e.data);if(Array.isArray(data))data.forEach(x=>{if(x?.s&&x.c)live.set(x.s,Number(x.c))})}catch{}};
