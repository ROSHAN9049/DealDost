/* Delta-style visual stabilizer + paper risk guard. Keeps scanner coins in live Delta order while values update, and prevents rapid revenge re-entry. */
(function(){
  ...
  function arrange(){
    ...
    /* Exact DeltaScanner display order: absolute 24H change descending. */
    const sorted=rows.slice().sort((a,b)=>Math.abs(num(b.querySelector('td:nth-child(3)')?.textContent))-Math.abs(num(a.querySelector('td:nth-child(3)')?.textContent)));
  ...
  function decorate(){
    ...
    rows.forEach(tr=>{const td=tr.querySelectorAll('td'),symbol=(td[0]?.textContent||'').trim();tr.style.display=search&&symbol.toUpperCase().indexOf(search)<0?'none':'';tr.classList.add('scannerRow');if(td[0]){const ch=num(td[2]?.textContent);td[0].classList.remove('coinUp','coinDown');td[0].classList.add(ch>=0?'coinUp':'coinDown')}if(td[2])td[2].classList.add(num(td[2].textContent)>=0?'up':'down');if(td[6]?.textContent.trim()==='LONG'||td[9]?.textContent.trim()==='LONG')tr.classList.add('rowLong');if(td[6]?.textContent.trim()==='SHORT'||td[9]?.textContent.trim()==='SHORT')tr.classList.add('rowShort')});
    const visible=rows.filter(tr=>tr.style.display!=='none'),info=$('scannerInfo');if(info)info.textContent=`${visible.length} coins · Delta 24H change order`;
    ...
    ['SYMBOL','PRICE','24H CHANGE','VOLUME','MOMENTUM','SCORE','SIGNAL','SCALPING','SCORE','SIGNAL','SIGNAL REASON','ACTION'].forEach((x,i)=>{const h=table?.querySelectorAll('thead th')?.[i];if(h)h.textContent=x});colorPnl();
  }
  function colorPnl(){document.querySelectorAll('#momentumPositions td,#scalpPositions td,#trades td,#netPnl,#avgPnl').forEach(el=>{if(!/₹/.test(el.textContent))return;const v=num(el.textContent);el.classList.remove('pnlPositive','pnlNegative','pnlZero');el.classList.add(v>0?'pnlPositive':v<0?'pnlNegative':'pnlZero')});document.querySelectorAll('#momentumPositions tr,#scalpPositions tr').forEach(tr=>{const side=tr.querySelector('td:nth-child(2)')?.textContent.trim(),coin=tr.querySelector('td:first-child');if(coin)coin.classList.add(side==='LONG'?'coinLong':side==='SHORT'?'coinShort':'')})}
  ...
  $('search')?.addEventListener('input',()=>requestAnimationFrame(decorate));installRiskGuard();setInterval(()=>{installRiskGuard();monitorClosedPositions()},1000);
})();