/* Binance scanner UI: stable Delta-style order without rolling. */
(function(){
  ...
  function deltaStyleRenderSignals(rows){
    ...
    const qualifying=list.filter(r=>r.mom?.signal==='LONG'||r.mom?.signal==='SHORT'||r.scalp?.signal==='LONG'||r.scalp?.signal==='SHORT').length;
    const set=window.set||((id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v});
    set('scannerInfo',`${visible.length} coins${search?' · filtered':''} · stable Delta-style order`);set('signalCount',`${qualifying} qualifying signals`);
  }
  // Current app.js analyses a maximum of 50 candle candidates. Render as soon as that batch is complete.
  window.renderSignals=function(rows){
    const universe=Number((document.getElementById('coinCount')?.textContent||'').split('/')[0])||0;
    const expected=Math.min(50,universe);
